...
export default function HomePage() {
  ...
  const uploadCopy = {
    en: { label: "Room photos", choose: "Select photos", help: "Optional. Upload 1–5 room photos if available." },
    ja: { label: "客室写真", choose: "写真を選択", help: "任意。写真があれば1〜5枚アップロードできます。" },
    zh: { label: "客房照片", choose: "选择照片", help: "选填。如方便，可上传1–5张客房照片。" },
  }[locale];
  const contactQr = {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); if (!validate(event.currentTarget)) return; setFormStatus("sending"); const response = await fetch("/api/review", { method: "POST", body: new FormData(event.currentTarget) }); if (response.ok) setFormStatus("success"); else setFormStatus("error"); }
