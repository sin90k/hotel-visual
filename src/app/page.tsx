...
const copy = {
  zh: {
    ...
    review: { title: "告诉我们想改善什么", body: "不一定需要先填写酒店名或网址。留下邮箱和想咨询的内容即可；如果方便，也可以上传几张客房照片帮助我们判断。", includes: ["3项优先优化建议", "1张示例参考图", "OTA页面展示检查", "客房图片调整方向"], fields: ["酒店或民宿名称", "房源或OTA链接", "联系邮箱", "微信或手机号", "想重点改善的内容"], button: "提交评估", sending: "正在提交…", success: "提交成功", successBody: "我们已收到您的咨询内容。确认后，工作人员会与您联系。", error: "表单邮件暂未接通，请先扫码添加微信或发送邮件联系。" },
  ...
  en: {
    ...
    review: { title: "Tell us what you want to improve", body: "You do not need to share the property name or URL first. Leave your email and a short message; room photos are optional but helpful for a faster review.", includes: ["3 photo optimization recommendations", "1 complimentary sample image", "OTA presentation analysis", "Room presentation recommendations"], fields: ["Property Name", "Property or OTA URL", "Email", "WhatsApp / Phone", "Message"], button: "Request Assessment", sending: "Submitting…", success: "Thank you", successBody: "We have received your request. After confirmation, our team will contact you with the assessment.", error: "The form email service is not connected yet. Please contact us by WhatsApp or email for now." },
  ...
  ja: {
    hero: { eyebrow: "客室写真・予約ページ改善", title: "客室の魅力が伝わる予約ページへ", body: "今ある写真をもとに、主写真・掲載順・明るさ・色味を見直します。予約前のゲストが客室を選びやすい状態へ整えます。", primary: "無料診断を申し込む", secondary: "改善事例を見る" },
    why: { title: "予約サイトで伝わりにくい理由", body: "写真の枚数や画質だけでなく、主写真・掲載順・客室情報の伝わり方が予約判断に影響します。", cards: [["主写真で魅力が伝わらない", "一覧画面で小さく表示されても、客室の特徴が伝わる写真へ整えます。"], ["写真の並びが分かりにくい", "ベッド、窓、浴室、設備が自然につながる順番へ整理します。"], ["撮り直しの判断が難しい", "今ある写真で整えられる点と、撮り足すべきカットを整理します。"]] },
    products: { title: "サービス", photo: { label: "中心となるサービス", title: "客室写真・予約ページ改善", body: "1枚 ¥200〜。新たにカメラマンを手配しなくても、既存写真を予約ページで使いやすい見え方へ整えます。必要に応じて、内装アップグレード案も画像で確認できます。", bullets: ["主写真の見直し", "写真掲載順の整理", "明るさと色味の調整", "広さと清潔感の表現", "OTA一覧画面での確認", "不足カットの整理"], cta: "写真改善について見る" }, feedback: { label: "滞在中の声を拾う仕組み", title: "ゲスト対応QR", body: "客室QRからスマートフォン用Webページを開き、届いた声を判定・担当部署へ共有。対応状況を記録し、完了まで追跡します。アプリは不要です。", bullets: ["客室QRとスマートフォン用Webページ", "リスク判定とタスク作成", "スタッフ対応と責任者の確認", "ゲスト確認と対応レポート"], cta: "ゲスト対応QRを見る", example: { label: "利用イメージ", room: "客室 1208", title: "ご意見：空調の音", status: "設備担当のタスクを作成・対応状況を確認中" } } },
    ...
    review: { title: "気になる点をお知らせください", body: "施設名やURLは必須ではありません。メールアドレスとご相談内容だけで送信できます。写真があれば、より具体的に確認できます。", includes: ["優先して直したいポイント3点", "改善イメージが分かるサンプル画像1点", "必要に応じた追加撮影の方向性", "予約サイト上での見え方の診断"], fields: ["施設名", "施設またはOTAページURL", "メールアドレス", "LINE ID / 電話番号", "ご相談内容"], button: "無料診断を申し込む", sending: "送信中…", success: "お申し込みありがとうございます", successBody: "ご相談内容を受け付けました。確認後、担当者よりご連絡します。", error: "フォーム送信は現在準備中です。お手数ですが、LINEまたはメールでご連絡ください。" },
...
export default function HomePage() {
  ...
  const uploadCopy = {
    en: { label: "Room photos", choose: "Add photos", selected: (count: number) => `${count}/5 selected`, remove: "Remove", help: "Optional. Add up to 5 room photos." },
    ja: { label: "客室写真", choose: "写真を追加", selected: (count: number) => `${count}/5枚を選択済み`, remove: "削除", help: "任意。最大5枚まで追加できます。" },
    zh: { label: "客房照片", choose: "添加照片", selected: (count: number) => `已选择 ${count}/5 张`, remove: "删除", help: "选填。最多添加 5 张客房照片。" },
  }[locale];
  const contactQr = {
  ...
  function validate(form: HTMLFormElement) {
    ...
    if (!message) nextErrors.message = validationCopy.required(c.review.fields[4]);
  ...
  async function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); if (!validate(event.currentTarget)) return; setFormStatus("sending"); const form = event.currentTarget; const formData = new FormData(form); formData.delete("photos"); selectedPhotos.forEach((photo) => formData.append("photos", photo.file, photo.name)); const response = await fetch("/api/review", { method: "POST", body: formData }); if (response.ok) { setFormStatus("success"); form.reset(); selectedPhotos.forEach((photo) => URL.revokeObjectURL(photo.url)); setSelectedPhotos([]); } else setFormStatus("error"); }
  return <main className="marketing-home">
    ...
    <section className="home-review" id="review"><div><h2>{c.review.title}</h2><p>{c.review.body}</p><ul>{c.review.includes.map(item => <li key={item}><Check />{item}</li>)}</ul></div>{formStatus === "success" ? <div className="home-success"><Check /><h3>{c.review.success}</h3><p>{c.review.successBody}</p></div> : <form onSubmit={submit} noValidate><label className="form-email"><span>{c.review.fields[2]} *</span><input name="email" type="email" aria-invalid={Boolean(formErrors.email)} placeholder="name@example.com" onChange={() => formErrors.email && setFormErrors((current) => ({ ...current, email: "" }))} />{formErrors.email && <small className="field-error">{formErrors.email}</small>}</label><label><span>{c.review.fields[0]}</span><input name={formFieldNames[0]} type="text" placeholder={locale==="ja" ? "例：ホテル青葉" : locale==="zh" ? "例如：青岚酒店" : "The Willow House"} /></label><label><span>{c.review.fields[1]}</span><input name={formFieldNames[1]} type="url" aria-invalid={Boolean(formErrors.websiteUrl)} placeholder="https://example.com" onChange={() => formErrors.websiteUrl && setFormErrors((current) => ({ ...current, websiteUrl: "" }))} />{formErrors.websiteUrl && <small className="field-error">{formErrors.websiteUrl}</small>}</label><label className="wide"><span>{c.review.fields[4]} *</span><textarea name="message" rows={3} aria-invalid={Boolean(formErrors.message)} placeholder={locale==="ja" ? "気になっている写真や客室があればお聞かせください。" : locale==="zh" ? "可以告诉我们目前最想改善哪类客房或哪组图片。" : "Tell us what you would like to improve."} onChange={() => formErrors.message && setFormErrors((current) => ({ ...current, message: "" }))} />{formErrors.message && <small className="field-error">{formErrors.message}</small>}</label><div className="review-contact-upload-row wide"><div className="contact-qr-card wide"><Image className="contact-qr-image" src={contactQr.image} alt={contactQr.alt} width={520} height={520} unoptimized /><div><strong>{contactQr.title}</strong><p>{contactQr.body}</p><a className="contact-qr-link" href={contactQr.href}>{contactQr.button}</a></div></div><label className="file-field file-field-compact wide"><span>{uploadCopy.label}</span><input type="file" accept="image/png,image/jpeg,image/webp" multiple onChange={(event) => { const files = Array.from(event.currentTarget.files || []); event.currentTarget.value = ""; addPhotos(files); }} /><div className="file-upload-box" aria-hidden="true"><b>{uploadCopy.choose}</b><small>{uploadCopy.selected(selectedPhotos.length)}</small></div><em>{uploadCopy.help}</em></label></div>{selectedPhotos.length > 0 && <div className="photo-preview-grid wide">{selectedPhotos.map((photo) => <figure key={photo.id}><img src={photo.url} alt={photo.name} /><button type="button" onClick={() => removePhoto(photo.id)}>{uploadCopy.remove}</button></figure>)}</div>}{formStatus === "error" && <p className="form-error wide">{c.review.error}</p>}<button className="button button-gold" type="submit">{formStatus === "sending" ? c.review.sending : c.review.button}<ArrowRight /></button></form>}</section>
