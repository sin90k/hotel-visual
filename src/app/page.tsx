...
const copy = {
  zh: {
    ...
    products: { title: "两项服务，一个目标：多接订单，少留客诉", photo: { label: "核心服务", title: "客房图片优化", body: "不用重新拍摄，也能把现有照片里的采光、空间和床品质感更好地展示出来。", bullets: ["调整明暗与光线", "统一整套图片色调", "突出空间纵深", "提升床品与材质质感", "优化OTA主图", "无需重新拍摄"], cta: "查看图片优化服务" }, feedback: { label: "客诉辅助工具", title: "住客反馈二维码", body: "住客有不满时先扫码联系酒店，给前台和运营团队留出及时补救的时间。", bullets: ["客诉直接发给酒店", "第一时间通知工作人员", "尽量在退房前处理", "无需下载应用"], cta: "查看住客反馈二维码" } },
  ...
  ja: {
    ...
    products: { title: "ご提供するサービス", photo: { label: "中心となるサービス", title: "客室写真の見せ方改善", body: "撮り直しをしなくても、今ある写真から客室本来の魅力を引き出せます。", bullets: ["自然な明るさに調整", "写真全体の色味を統一", "奥行きと広がりを表現", "寝具の清潔感と質感を強調", "予約サイトのメイン写真を最適化", "再撮影は不要"], cta: "写真改善について見る" }, feedback: { label: "滞在中の声を拾う仕組み", title: "ご意見受付QR", body: "小さな不満が口コミになる前に、ゲストから施設へ直接伝えてもらえます。", bullets: ["施設へ直接届く", "滞在中に対応できる", "低評価口コミを未然に防ぐ", "アプリのダウンロード不要"], cta: "ご意見受付QRについて見る" } },
...
export default function HomePage() {
  ...
  return <main className="marketing-home">
    ...
    <section className="home-products"><h2>{c.products.title}</h2><div className="home-product-grid"><article className="home-photo-product"><span>{c.products.photo.label}</span><h3>{c.products.photo.title}</h3><p>{c.products.photo.body}</p><ul>{c.products.photo.bullets.map(item => <li key={item}><Check />{item}</li>)}</ul><Link href="/products/photo-enhancement">{c.products.photo.cta}<ArrowRight /></Link></article><article className="home-feedback-product"><QrCode /><span>{c.products.feedback.label}</span><h3>{c.products.feedback.title}</h3><p>{c.products.feedback.body}</p><ul>{c.products.feedback.bullets.map(item => <li key={item}><Check />{item}</li>)}</ul><Link href="/products/guest-feedback">{c.products.feedback.cta}<ArrowRight /></Link></article></div></section>
